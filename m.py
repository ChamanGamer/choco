...
bot = telebot.TeleBot('7429297419:AAHOfZEPKl474CbP9vifohxkooboQT-skZg')
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, HAMLA CHALU attack PE🚀.\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: attack\nBy ACE MODS "
    bot.reply_to(message, response)
...
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in attack_cooldown and (datetime.datetime.now() - attack_cooldown[user_id]).seconds < 30:
